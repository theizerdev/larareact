...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['roles'] ?? 'roles', function (Blueprint $table) use ($teams, $columnNames) {
            ...
            $table->unsignedBigInteger($teamKey)->nullable()->index('roles_team_foreign_key_index');
            $table->string('name');
            $table->string('guard_name');
            $table->timestamps();
        ...
        Schema::create($tableNames['model_has_permissions'] ?? 'model_has_permissions', function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission, $teams) {
            ...
            $teamKey = $columnNames['team_foreign_key'] ?? 'empresa_id';
            $table->unsignedBigInteger($teamKey)->default(0)->index('model_has_permissions_team_foreign_key_index');

            $table->primary([$teamKey, $pivotPermission, $columnNames['model_morph_key'] ?? 'model_id', 'model_type'], 'model_has_permissions_permission_model_type_primary');
        });
        ...
        Schema::create($tableNames['model_has_roles'] ?? 'model_has_roles', function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole, $teams) {
            ...
            $teamKey = $columnNames['team_foreign_key'] ?? 'empresa_id';
            $table->unsignedBigInteger($teamKey)->default(0)->index('model_has_roles_team_foreign_key_index');

            $table->primary([$teamKey, $pivotRole, $columnNames['model_morph_key'] ?? 'model_id', 'model_type'], 'model_has_roles_role_model_type_primary');
        });
