...
class WhatsAppAPIServer {
  ...
  async initialize() {
    try {
      ...
      // Inicializar administrador multi-empresa de WhatsApp
      this.whatsappManager = new WhatsAppManager(this.io);
      await this.whatsappManager.initializeAllActiveCompanies();
      ...
      queueService.setWhatsAppManager(this.whatsappManager);
  ...
  async gracefulShutdown() {
    ...
    try {
      ...
      if (this.whatsappManager) {
        await this.whatsappManager.shutdownAll();
        logger.info('Servicios WhatsApp detenidos');
      }
