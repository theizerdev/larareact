...
import { ArrowLeft, Printer, Download, Send, Sprout } from 'lucide-react';
...
interface CarnetProductorPageProps {
    productor: Productor;
}

export default function CarnetProductorPage({ productor }: CarnetProductorPageProps) {
    const { __ } = useTranslate();
    const { auth } = usePage().props as any;
    const [downloading, setDownloading] = useState(false);
    const [sendingWhatsapp, setSendingWhatsapp] = useState(false);

    const displayName = productor.nombre_comercial || productor.razon_social;
    ...
    const handleDownloadImage = async () => {
        ...
        try {
            ...
            link.href = image;
            link.download = `Gafete_Productor_${displayName.replace(/\s+/g, '_')}.png`;
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-slate-100 dark:bg-slate-900 py-8 px-4 flex flex-col items-center justify-center">
                ...
                {/* ── Credencial (Gafete Azul de Productor) ── */}
                <div
                    id="badge-wrapper"
                    style={{
                        width: '340px',
                ...
                >
                    {/* Abstract watercolor top spots en tonos azules e índigo */}
                    <div style={{ position: 'absolute', top: '-8px', left: '-5px', width: '95px', height: '60px', backgroundColor: 'rgba(29, 78, 216, 0.95)', borderRadius: '45% 55% 70% 30% / 50% 60% 40% 50%', filter: 'blur(1.5px)', mixBlendMode: 'multiply' }} />
                    <div style={{ position: 'absolute', top: '-12px', left: '80px', width: '85px', height: '58px', backgroundColor: 'rgba(14, 165, 233, 0.9)', borderRadius: '50% 40% 60% 40% / 40% 50% 50% 60%', filter: 'blur(1.5px)', mixBlendMode: 'multiply' }} />
                    <div style={{ position: 'absolute', top: '-15px', left: '155px', width: '90px', height: '52px', backgroundColor: 'rgba(67, 56, 202, 0.85)', borderRadius: '30% 70% 40% 60% / 50% 40% 60% 50%', filter: 'blur(1.5px)', mixBlendMode: 'multiply' }} />

                    {/* ══ Sección Superior (Nombre y Badge de Productor) ══ */}
                    <div
                        style={{
                            display: 'flex',
                            flexDirection: 'row',
                    ...
                    >
                        ...
                        {/* Icono / Marca de Productor en marco azul */}
...
CarnetProductorPage.layout = (page: React.ReactNode) => page;
