...
interface Props {
    ...
    onItemsUpdated?: (grouped: Record<string, ChecklistItem[]>) => void;
}
...
export default function ChecklistConfigModal({
    ...
    const [activeSection, setActiveSection] = useState<'validacion' | 'limpieza' | 'qc'>('validacion');
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-3">
                            <div className="space-y-1">
                                <DialogTitle className="text-base font-black flex items-center gap-2 text-slate-900 dark:text-slate-100">
                                    ...
                                    {__('Configuración de Ficha de Validación & Post-Atención')}
                                </DialogTitle>
                                <DialogDescription className="text-xs text-slate-500">
                                    {__(
                                        'Personalice los puntos de revisión por empresa o por sucursal: agregue, edite, ordene o active/desactive.'
                                    )}
                                </DialogDescription>
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                            <div className="flex flex-wrap sm:flex-nowrap w-full sm:w-auto h-auto bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl gap-1">
                                <button
                                    type="button"
                                    onClick={() => setActiveSection('validacion')}
                                    className={cn(
                                ...
                                >
                                    ...
                                    <span>{__('1. Validación')}</span>
                                    <Badge className="ml-1 text-[10px] px-1.5 py-0 bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-200">
                                        {items.filter((i) => i.seccion === 'validacion' && i.activo).length}/
                                        {items.filter((i) => i.seccion === 'validacion').length}
                                    </Badge>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => setActiveSection('limpieza')}
                                    className={cn(
                                        'flex-1 sm:flex-none flex items-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer',
                                ...
                                >
                                    ...
                                    <span>{__('2. Limpieza & Protocolo')}</span>
                                    <Badge className="ml-1 text-[10px] px-1.5 py-0 bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">
                                        {items.filter((i) => i.seccion === 'limpieza' && i.activo).length}/
                                        {items.filter((i) => i.seccion === 'limpieza').length}
                                    </Badge>
                                </button>
                                <button
                                    type="button"
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Button
                                    ...
                                    className="h-8 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold gap-1.5 rounded-lg shadow-xs"
                                >
                        ...
                        <div className="pt-2">
                            {isLoading ? (
                                <div className="flex flex-col items-center justify-center py-12 text-slate-400 gap-2">
                                    ...
                                    <span className="text-xs">{__('Cargando puntos de control...')}</span>
                                </div>
            ...
            <Dialog open={!!itemToDelete} onOpenChange={(open) => !open && setItemToDelete(null)}>
                <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-3xl p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-slate-500">
                            {__('Se eliminará')} <strong>"{itemToDelete?.nombre}"</strong> {__('de la plantilla.')}
                        </DialogDescription>
                    </DialogHeader>
            ...
            <Dialog open={openResetModal} onOpenChange={setOpenResetModal}>
                <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-3xl p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-slate-500">
                            {__(
                                'Se restaurarán todos los puntos por defecto del sistema y se eliminarán las personalizaciones.'
                            )}
                        </DialogDescription>
                    </DialogHeader>
